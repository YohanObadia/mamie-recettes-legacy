---
layout: page
title: Tiramissou
meal_type: Dessert
Chef:
  person: Radio Y
  note: ''
portions: 8
prep_time_minutes: 25
cook_time_minutes: 0
image:
  path: /assets/images/tiramissou/main.jpg
  thumbnail: /assets/images/tiramissou/main.jpg
  photos:
  - /assets/images/tiramissou/main.jpg
video:
  type: youtube
  url: ''
ingredients:
- name: Sucre en poudre
  quantity: '50'
  unit: g
  note: ''
- name: Jaunes d’œufs
  quantity: '3'
  unit: ''
  note: ''
- name: Blancs d’œufs
  quantity: '2'
  unit: ''
  note: ''
- name: Mascarpone
  quantity: '1'
  unit: ''
  note: (à confirmer)
- name: Biscuits à la cuillère
  quantity: '40'
  unit: ''
  note: ''
- name: Café
  quantity: '5'
  unit: tasses
  note: ''
- name: Rhum
  quantity: ''
  unit: ''
  note: un peu
- name: Cacao en poudre
  quantity: '5'
  unit: c. à soupe
  note: ''
---

{% include recipe-meta.html %}

## Ingrédients
- 50 g de sucre en poudre
- 3 jaunes d’œufs
- 2 blancs d’œufs
- 1 mascarpone (quantité à confirmer)
- 40 biscuits à la cuillère
- 5 tasses de café
- Un peu de rhum
- 5 c. à soupe de cacao en poudre

## Étapes
1. Battre les jaunes d’œufs avec le sucre, puis ajouter le mascarpone.
2. Monter les blancs en neige et les incorporer délicatement à la crème.
3. Mélanger le café et le rhum.
4. Imbiber rapidement les biscuits dans le café.
5. Monter en couches : biscuits, crème, cacao, biscuits, crème, cacao.
6. Mettre au frais au moins 4 heures (idéalement une nuit).